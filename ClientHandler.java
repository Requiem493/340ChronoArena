    ...
    public void run() {
        try {
            ...
            while (true) {
                ...
                System.out
                        .println("[Client] Request: " + request.substring(0, Math.min(100, request.length())) + "...");
    ...
    private void handleTaskRequest(String request, BufferedReader in, BufferedWriter out) throws IOException {
        ...
        if (serviceName.equals("CSV") && taskData.equals("CHUNKED")) {
            ...
            try {
                ...
                System.out
                        .println("[Client] Streamed " + chunkCount + " chunks to service node, waiting for result...");

                // Get result from service node
                String result = snIn.readLine();
                if (result == null)
                    result = "ERROR|500|Service unavailable";
